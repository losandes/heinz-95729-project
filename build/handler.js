...
function parse$a(string) {
  ...
  var type2 = index2 !== -1 ? header3.slice(0, index2).trim() : header3.trim();
...
var ReadStream = require$$1__default.ReadStream;
...
function requireStreams() {
  ...
  var Buffer2 = require$$0$6.Buffer, Transform = require$$1$2.Transform;
...
function requireRawBody() {
  ...
  function getRawBody(stream2, options, callback) {
    ...
    if (done) {
      return readStream(stream2, encoding3, length, limit, wrap2(done));
    }
    return new Promise(function executor(resolve3, reject) {
      readStream(stream2, encoding3, length, limit, function onRead(err, buf) {
        if (err)
          return reject(err);
        resolve3(buf);
      });
  ...
  function readStream(stream2, encoding3, length, limit, callback) {
    var complete = false;
    var sync = true;
...
function parse$9(string) {
  ...
  var type2 = index2 !== -1 ? string.substr(0, index2) : string;
  var key;
  var match2;
...
function typeis$1(value, types_) {
  ...
  var type2;
  for (i = 0; i < types.length; i++) {
    if (mimeMatch(normalize$1(type2 = types[i]), val)) {
...
function normalize$1(type2) {
  ...
  return type2.indexOf("/") === -1 ? mime$4.lookup(type2) : type2;
}
...
function requireJson() {
  ...
  function typeChecker(type2) {
    return function checkType(req2) {
      return Boolean(typeis2(req2, type2));
    };
  }
  return json_1;
}
...
var sentinel = {};
var stringify$2 = function stringify(object, prefix, generateArrayPrefix, commaRoundTrip, strictNullHandling, skipNulls, encoder, filter, sort, allowDots, serializeDate2, format2, formatter, encodeValuesOnly, charset3, sideChannel2) {
  var obj = object;
...
function finalhandler(req2, res2, options) {
  ...
  return function(err) {
    ...
    debug$5("default %s", status3);
    if (err && onerror) {
      defer(onerror, err, req2, res2);
    }
    if (headersSent$1(res2)) {
      debug$5("cannot %d after headers sent", status3);
      req2.socket.destroy();
      return;
    }
...
Route$1.prototype.all = function all() {
  ...
  for (var i = 0; i < handles.length; i++) {
    ...
    if (typeof handle3 !== "function") {
      var type2 = toString$2.call(handle3);
...
proto.handle = function handle2(req2, res2, out) {
  var self2 = this;
  debug$2("dispatching %s %s", req2.method, req2.url);
  var idx = 0;
  ...
  function trim_prefix(layer2, layerError, layerPath, path3) {
    if (layerPath.length !== 0) {
      ...
      debug$2("trim prefix (%s) from url %s", layerPath, req2.url);
      removed = layerPath;
    ...
    debug$2("%s %s : %s", layer2.name, layerPath, req2.originalUrl);
    if (layerError) {
...
proto.use = function use(fn) {
  ...
  for (var i = 0; i < callbacks.length; i++) {
    ...
    debug$2("use %o %s", path3, fn.name || "<anonymous>");
...
var Stats = require$$1__default.Stats;
...
SendStream.prototype.type = function type(path3) {
  ...
  if (res2.getHeader("Content-Type"))
    return;
  var type2 = mime$2.lookup(path3);
  if (!type2) {
    debug("no content-type");
    return;
  }
  var charset3 = mime$2.charsets.lookup(type2);
  debug("content-type %s", type2);
  res2.setHeader("Content-Type", type2 + (charset3 ? "; charset=" + charset3 : ""));
};
...
function parseipNotation(note) {
  ...
  } else if (ip2.kind() === "ipv4" && isip(range2)) {
...
(function(exports) {
  ...
  var qs2 = lib;
  var querystring = require$$8__default;
  ...
  exports.setCharset = function setCharset2(type2, charset3) {
    if (!type2 || !charset3) {
      return type2;
    }
    var parsed = contentType$1.parse(type2);
...
(function(module, exports) {
  ...
  app2.route = function route3(path3) {
    this.lazyrouter();
...
const app = express$1();
app.get("/api/hello", (_req, res2) => {
  res2.send({ hello: "world" });
});
app.get("/api/palettes", async (_req, res2) => {
  const palettes2 = await loadPalettes();
  res2.send(palettes2);
});
const handler = app;
export {

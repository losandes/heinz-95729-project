 ...
 * - `ALLOW_DEV_CONFIGURATIONS`: turn dev features on and off, such as GraphiQL, and the Content-Security-Policies that go with that
 * - `APP_VERSION`: the version as stated in the package.json file
 * - `PORT`: the port this app can be reached at: default='3001'
 * - `ROUTER_PREFIX`: the path this api can be reached at when it's running in proxy mode: default=''
 ...
 * - `JWT_EXPIRES_IN`: JWT Expiry in the format that jsonwebtoken wants it: make sure it matches  JWT_EXPIRES_IN_MS!: default='30d'
 * - `JWT_EXPIRES_IN_MS`: Cookie Expiry in the millisecond format that koa's ctx.cookies.set wants it: make sure it matches JWT_EXPIRES_IN!: default=(86400000 * 30) (30 days)
 * - `LOG_WRITER`: the typeof writer to emit logs to: default='DevConsoleWriter'] - /^(ArrayWriter|ConsoleWriter|DevConsoleWriter|StdoutWriter)$/
 * - `LOG_FORMATTER`: the typeof formatter to format the logs with: default='BlockFormatter'] - /^(BlockFormatter|BunyanFormatter|JsonFormatter|PassThroughFormatter|StringFormatter|SquashFormatter)$/
 * - `LOG_LISTENERS`: the log events to listen for: default=['local', 'todo', 'trace', 'debug', 'info', 'warn', 'error', 'fatal']]

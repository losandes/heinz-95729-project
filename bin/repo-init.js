...
const MESSAGES = {
  ...
  SKIPPED_FILE: {
    EXISTS: 'SKIPPED FILE (already exists):',
    SOURCE_MISSING: 'SKIPPED FILE (source file does not exist):',
  }
}
...
const PATHS = {
  DATA_VOLUMES: path.join(process.cwd(), 'server', 'data_volumes'),
  DATA_FILE: path.join(process.cwd(), 'server', 'data_volumes', 'heinz-95729.db'),
  SERVER_ENV_EXAMPLE: path.join(process.cwd(), 'server', '.env.example'),
  SERVER_ENV: path.join(process.cwd(), 'server', '.env'),
